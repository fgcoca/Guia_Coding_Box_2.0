???+ Danger ""
    !!! Info "Aviso:"
        Todo lo que se explica en esta entrada MicroPython se realiza en entorno Linux bajo Sistema Operativo Ubuntu 22.04 LTS
...
* Sin embargo lo práctico gana a lo puro.
* Los errores nunca deberían dejarse pasar silenciosamente.
* A menos que hayan sido silenciados explícitamente.
* Frente a la ambigüedad, rechaza la tentación de adivinar.
* Debería haber una, y preferiblemente sólo una, manera obvia de hacerlo.
* Aunque esta forma de hacerlo puede no resultar obvia al principio, a menos que seas holandés (en clara referencia a su autor).
...
El [Zen de Python](https://es.wikipedia.org/wiki/Zen_de_Python) viene incorporado (en inglés) a partir de la versión 2.1.2 y podemos invocarlo con import this abriendo Python en una terminal en un sistema en el que lo tengamos instalado, lógicamente.

En la imagen siguiente podemos ver como se ejecuta Python3 y como se incova al Zen de Python para mostrar "*The Zen of Python, by Tim Peters*":

![The Zen of Python, by Tim Peters](../img/Pyth/Zen.png){.center-img100}
...
* Damos siempre a los identificadores un nombre que tenga sentido. Aunque que ```c = 10``` es perfectamente válido, escribir ```contador = 10``` tendría más sentido, y sería más fácil averiguar lo que representa cuando miremos el código pasado un tiempo.
...
En la imagen siguiente se representa el concepto de variable como recipientes con nombre (para identificar la variable) y su contenido, que representa el valor de la variable.

![Concepto de variable](../img/Pyth/concep_var.png){.center-img75}

La variable ```Nombre``` contiene la cadena "Juan", la variable ```Hombre``` es cierta y la variable ```Dorsal``` contiene el valor ```125```.
...
!!! Abstract "Reglas para nombrar variables"
...
* Booleanos. Solamente hay dos literales booleanos ```True``` y ```False```
...
En Python existe un literal especial, ```None```. Podemos usarlo, por ejemplo, para especificar una variable nula:
...
![Cadenas en el IDLE](../img/Pyth/cad1.png){.center-img75}

Una cadena ```raw``` (cruda) se interpreta tal como se escribe, es decir, no se omiten los caracteres especiales expresados con \. Las cadenas ```raw``` se escriben entrecomilladas y van precedidas del carácter 'r o R'. Es un tipo de cadena de texto que ignora los caracteres de escape, como \n (salto de línea) o \t (tabulación). En la imagen vemos un ejemplo.

![Cadena cruda en el IDLE](../img/Pyth/cad2.png){.center-img75}
...
![Concatenar y multiplicar textos en el IDLE](../img/Pyth/cad3.png){.center-img75}
...
![Ejemplos en el IDLE de operadores lógicos](../img/Pyth/logicos.png){.center-img100}
...
Los operadores de membresía (```in``` y ```not in```) en Python se utilizan para verificar si un valor o elemento específico existe dentro de una secuencia o colección (como listas, cadenas, tuplas o diccionarios). Devuelven un valor booleano (True o False).

El operador ```in```devuelve ```True``` si el elemento buscado se encuentra dentro de la secuencia, y ```False``` en caso contrario.

```python
# Buscar en una lista
frutas = ["manzana", "pera", "naranja"]
print("manzana" in frutas)  # Devuelve: True
print("plátano" in frutas)  # Devuelve: False

# Buscar en una cadena de texto (string)
texto = "Hola mundo"
print("mundo" in texto)     # Devuelve: True
print("Python" in texto)    # Devuelve: False
```

El operador ```not in```devuelve ```True``` si el elemento buscado no está presente en la secuencia, y ```False``` si sí se encuentra.

```python
numeros = [1, 2, 3, 4, 5]

print(10 not in numeros)    # Devuelve: True (10 no está en la lista)
print(3 not in numeros)     # Devuelve: False (3 sí está en la lista)
```
